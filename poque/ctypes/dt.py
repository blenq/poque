...
def read_reltime_bin(crs, length):
    return timedelta(seconds=crs.advance_int4())
...
def read_time_bin(crs, length):
    return time(*_time_vals_from_int(crs.advance_int8()))


def read_timetz_bin(crs, length):
    jd, seconds = crs.advance_struct_format_qi()
...
def read_interval_bin(crs, length):
    usecs, days, months = crs.advance_struct_format_qii()
